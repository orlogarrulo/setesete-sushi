...
//#region node_modules/.nitro/vite/services/ssr/assets/ops.functions-LekjFI0_.js
...
async function createPgliteSql() {
	...
	const migrate = async () => {
		const migrations = /* #__PURE__ */ Object.assign({
			"/migrations/0002_ops.sql": _0002_ops_default,
...
async function requireStaff() {
	const { requireStaff: check } = await import("./staff-session-Bdj_mhoq.mjs");
	check();
}
function dbFail(err) {
...
async function markReceiptFlags(sql, orders) {
	if (orders.length === 0) return orders;
	const recs = await sql`select distinct order_id from receipts`;
	const has = new Set(recs.map((r) => r.order_id));
	return orders.map((o) => ({
		...o,
		hasReceipt: has.has(o.id) || Boolean(o.receiptName)
	}));
}
async function saveReceipt(sql, opts) {
	const id = newId("rc");
...
var createOrder = createServerFn({ method: "POST" }).validator(object({
	...
	afterHours: boolean().optional().default(false),
	...
	if (data.receipt) await saveReceipt(sql, {
		orderId: data.id,
		customerId,
		phone: data.phone,
	...
	const note = data.afterHours ? "Pedido fora do horário. A confirmar na abertura, por ordem de chegada." : data.receipt ? "Pedido chegou pelo site. Comprovativo anexado." : "Pedido chegou pelo site. Fatura gerada — a aguardar pagamento.";
...
var staffSession = createServerFn({ method: "GET" }).handler(staffSession_createServerFn_handler, async () => {
	const { readStaffSession } = await import("./staff-session-Bdj_mhoq.mjs");
...
var loginStaff = createServerFn({ method: "POST" }).validator(object({
	...
	const { attemptStaffLogin } = await import("./staff-session-Bdj_mhoq.mjs");
...
var logoutStaff = createServerFn({ method: "POST" }).handler(logoutStaff_createServerFn_handler, async () => {
	const { clearStaffCookie } = await import("./staff-session-Bdj_mhoq.mjs");
	clearStaffCookie();
	return { ok: true };
});
...
var listOrders = createServerFn({ method: "GET" }).validator(object({ status: string().optional() })).handler(listOrders_createServerFn_handler, async ({ data }) => {
	await requireStaff();
	const sql = await sqlReady();
	return markReceiptFlags(sql, (data.status ? await sql`select * from orders where status = ${data.status} order by created_at desc` : await sql`select * from orders order by created_at desc`).map(mapOrder));
});
...
var getOrder = createServerFn({ method: "GET" }).validator(object({ id: string() })).handler(getOrder_createServerFn_handler, async ({ data }) => {
	...
	const [withFlag] = await markReceiptFlags(sql, [mapOrder(rows[0])]);
	return {
		...withFlag,
		events: pub.events,
...
var findOrders_createServerFn_handler = createServerRpc({
	id: "090b8c82772100e5c03070b36dd145f30f77c7d016b34d50d105d5e1835b1518",
	name: "findOrders",
	filename: "src/lib/ops.functions.ts"
}, (opts) => findOrders.__executeServer(opts));
var findOrders = createServerFn({ method: "GET" }).validator(object({ q: string().min(2).max(40) })).handler(findOrders_createServerFn_handler, async ({ data }) => {
	await requireStaff();
	const sql = await sqlReady();
	const q = data.q.trim();
	return markReceiptFlags(sql, (await sql`
      select * from orders
      where id ilike ${"%" + q + "%"}
         or phone ilike ${"%" + q + "%"}
         or customer_name ilike ${"%" + q + "%"}
      order by created_at desc
      limit 20`).map(mapOrder));
});
var listReceipts_createServerFn_handler = createServerRpc({
	id: "156f710f63f255b6e16cb315bb8628ce6cb13c87af810a89809d0d6bd2856fb7",
	name: "listReceipts",
	filename: "src/lib/ops.functions.ts"
...
var listReceipts = createServerFn({ method: "GET" }).validator(object({
...
})).handler(listReceipts_createServerFn_handler, async ({ data }) => {
	await requireStaff();
	const rows = await (await sqlReady())`
	...
	return rows.filter((r) => {
		...
		if (!q) return true;
		return r.order_id.toLowerCase().includes(q) || r.phone.toLowerCase().includes(q) || r.customer_name.toLowerCase().includes(q) || phoneQ.length >= 6 && digitsPhone(r.phone).includes(phoneQ);
	}).map((r) => ({
		id: r.id,
		orderId: r.order_id,
...
var getReceipt_createServerFn_handler = createServerRpc({
	id: "3793b6eee164265ea5c215b5d2a658b3ef7ef43ee026711f9974483c8bd1de95",
	name: "getReceipt",
	filename: "src/lib/ops.functions.ts"
}, (opts) => getReceipt.__executeServer(opts));
var getReceipt = createServerFn({ method: "GET" }).validator(object({ orderId: string().min(4) })).handler(getReceipt_createServerFn_handler, async ({ data }) => {
	await requireStaff();
	return (await (await sqlReady())`select * from receipts where order_id = ${data.orderId} order by created_at desc`).map((r) => ({
		id: r.id,
		orderId: r.order_id,
		customerId: r.customer_id,
		phone: r.phone,
...
var submitOrderReceipt_createServerFn_handler = createServerRpc({
	id: "e808ac69a6c5e49c5813c24e9ab6144009a138ec6973699913d4d2e4d02adb37",
	name: "submitOrderReceipt",
	filename: "src/lib/ops.functions.ts"
}, (opts) => submitOrderReceipt.__executeServer(opts));
var submitOrderReceipt = createServerFn({ method: "POST" }).validator(object({
	token: string().min(4).max(64),
	receipt: receiptSchema
})).handler(submitOrderReceipt_createServerFn_handler, async ({ data }) => {
	const sql = await sqlReady();
	const rows = await sql`select * from orders where track_token = ${data.token} limit 1`;
	if (!rows[0]) throw new Error("Encomenda não encontrada.");
...
var attachReceipt_createServerFn_handler = createServerRpc({
	id: "8d63bf61709d7ee3c8cf157b71c82f91225ea174961c34c97411fbe09835b616",
	name: "attachReceipt",
	filename: "src/lib/ops.functions.ts"
}, (opts) => attachReceipt.__executeServer(opts));
var attachReceipt = createServerFn({ method: "POST" }).validator(object({
	orderId: string().min(4),
	receipt: receiptSchema
})).handler(attachReceipt_createServerFn_handler, async ({ data }) => {
	await requireStaff();
	const sql = await sqlReady();
	const rows = await sql`select * from orders where id = ${data.orderId} limit 1`;
	if (!rows[0]) throw new Error("Encomenda não encontrada.");
	await saveReceipt(sql, {
		orderId: rows[0].id,
		customerId: rows[0].customer_id,
...
var verifyPayment_createServerFn_handler = createServerRpc({
	id: "d1c961430f01f84389693c862ec7f436df68576de9ded80eb41c6433a48e9dce",
	name: "verifyPayment",
	filename: "src/lib/ops.functions.ts"
...
var verifyPayment = createServerFn({ method: "POST" }).validator(object({
	id: string().min(4),
	ok: boolean()
})).handler(verifyPayment_createServerFn_handler, async ({ data }) => {
	await requireStaff();
	const sql = await sqlReady();
	const now = iso(/* @__PURE__ */ new Date());
	if (data.ok) {
	...
	} else await sql`update orders set pay_verified = ${false}, pay_verified_at = ${null}, updated_at = ${now} where id = ${data.id}`;
	const rows = await sql`select * from orders where id = ${data.id} limit 1`;
...
//#endregion
export { addCrmNote_createServerFn_handler, attachReceipt_createServerFn_handler, createOrder_createServerFn_handler, findOrders_createServerFn_handler, getCustomer_createServerFn_handler, getDashboard_createServerFn_handler, getOrder_createServerFn_handler, getReceipt_createServerFn_handler, getTracking_createServerFn_handler, listCustomers_createServerFn_handler, listOrders_createServerFn_handler, listReceipts_createServerFn_handler, loginStaff_createServerFn_handler, logoutStaff_createServerFn_handler, lookupInvoice_createServerFn_handler, setCustomerTags_createServerFn_handler, setOrderStatus_createServerFn_handler, staffSession_createServerFn_handler, submitOrderReceipt_createServerFn_handler, upsertCustomer_createServerFn_handler, verifyPayment_createServerFn_handler };
