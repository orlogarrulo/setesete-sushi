import { a as STAFF_PIN, c as firstName, d as routeProgress, s as digitsPhone, t as COURIERS, u as remainingMinutes } from "./ops-BbesVQD-.mjs";
...
//#region node_modules/.nitro/vite/services/ssr/assets/ops.functions-Cul21KAt.js
...
function requirePin(pin) {
	if (pin !== "7700") throw new Error("PIN inválido.");
}
...
var lookupInvoice = createServerFn({ method: "GET" }).validator(object({ id: string().min(4).max(40) })).handler(lookupInvoice_createServerFn_handler, async ({ data }) => {
	const sql = await getSql();
	await ensureSeeded(sql);
	const rows = await sql`select track_token from orders where id = ${data.id.trim()} limit 1`;
	if (!rows[0]) return null;
	return { token: rows[0].track_token };
});
var verifyStaffPin_createServerFn_handler = createServerRpc({
	id: "2c70554e1aeccb32077cd822784799c32f2feb3b90f6fb139f15b151bf70e61d",
	name: "verifyStaffPin",
	filename: "src/lib/ops.functions.ts"
}, (opts) => verifyStaffPin.__executeServer(opts));
var verifyStaffPin = createServerFn({ method: "POST" }).validator(object({ pin: string() })).handler(verifyStaffPin_createServerFn_handler, async ({ data }) => {
...
})).handler(listOrders_createServerFn_handler, async ({ data }) => {
	requirePin(data.pin);
	const sql = await getSql();
	await ensureSeeded(sql);
...
var getCustomer = createServerFn({ method: "GET" }).validator(object({
	pin: string(),
	id: string()
})).handler(getCustomer_createServerFn_handler, async ({ data }) => {
	requirePin(data.pin);
	const sql = await getSql();
	await ensureSeeded(sql);
...
var getDashboard = createServerFn({ method: "GET" }).validator(object({ pin: string() })).handler(getDashboard_createServerFn_handler, async ({ data }) => {
	requirePin(data.pin);
	const sql = await getSql();
	await ensureSeeded(sql);
